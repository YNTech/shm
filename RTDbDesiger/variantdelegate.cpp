        ...
        const QModelIndex &index) const
{
    ...
    QVariant originalValue = index.model()->data(index);
                                   ...
                                   const QModelIndex &index) const
{
	...
	QWidget* wg = editor->parentWidget()->parentWidget()->parentWidget()->parentWidget();
	(qobject_cast<RTDbDesiger*>(wg))->m_flag = true;
    model->setData(index, displayText(value), Qt::EditRole);
